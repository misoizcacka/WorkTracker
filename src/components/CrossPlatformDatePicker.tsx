...
const CrossPlatformDatePicker: React.FC<CrossPlatformDatePickerProps> = ({ date, onDateChange }) => {
  ...
  if (Platform.OS === 'web') {
    ...
    const CustomInput = React.forwardRef<HTMLButtonElement, { value?: string; onClick?: () => void }>(({ value, onClick }, ref) => (
      <TouchableOpacity style={styles.button} onPress={onClick}>
        <Text style={styles.buttonText}>{value || 'Select Date'}</Text>
      </TouchableOpacity>
    ));
  ...
  return (
    <View>
      <TouchableOpacity style={styles.button} onPress={showPicker}>
        <Text style={styles.buttonText}>{moment(date).format('MMMM D, YYYY')}</Text>
      </TouchableOpacity>
      <DateTimePickerModal
